...
- 市内交通类型以 OCR 结果作为默认值，员工可在编辑费用时改正；最终选择为出租车/网约车时显示行程单关联入口，网约车缺少行程单会阻止提交，同一份多行程文件可被多笔费用分别选用。材料持久区分行程单、付款凭证和普通附件。OCR 未识别出具体说明时使用费用类别名称作为说明，不要求员工重复填写。
- 单张票据的员工确认人民币金额严格超过 500 元时需要关联付款凭证；500 元整不要求。目前铁路豁免仅限明确的 `high_speed`，未把全部火车票作为豁免。员工可把识别为高铁的票种改成动车或普通列车并采用更严格的付款凭证规则，但不能把已识别的动车或普通列车改成高铁取得豁免；最终铁路范围待业务确认。
- 国外票据保存原币信息，人民币报销金额由员工另行填写确认；币种不明确时仍需确认。员工修正或清空原币币种、金额后，以该人工结果为准，后续保存和恢复不会再用旧识别值覆盖。
- OCR 结果可编辑、删除，也可完全手工新增费用明细。重新识别只更新文件证据；费用行已经有人工修改时保留现有明细，不用新结果自动覆盖。
...
- 查询并复核当前员工已通过的出差审批，将选中实例写入 OA 的 `RelateField`。选择后，所属公司、预算代码、出差类别和日期会从同一候选立即展示；服务器核验期间保持固定布局并禁用重复选择，不额外插入瞬时状态标签，失败时显示真实错误。关联保存不等待普通输入的防抖时间，最终值仍以后端复核结果为准。同一报销中的审批必须来自同一发起部门，并且公司、预算和来源出差类别一致，日期允许不连续；每张审批保留独立关联记录，重叠日期在补助计算和 Excel 明细中自动合并。报销部门始终取员工当前有效部门：单部门直接使用该部门；多部门时优先匹配审批发起部门，历史部门已不在当前列表时才由员工从当前部门中选择。当前部门候选排除名称以“其他”开头的部门，并按名称去重。审批详情读取在同一后端进程内跨用户统一限速；钉钉以 HTTP 429 或 `QpsLimitForAppkeyAndApi` 返回限流时，安全读取会有限退避重试，耗尽后明确提示接口繁忙，不再误报应用权限不足。
...
- `POST /api/me/department/from-travel-approval`：核验出差审批并按当前有效部门规则确定本次报销部门；历史发起部门无法匹配且存在多个当前部门时，返回过滤、去重后的部门候选供员工确认。
...
- `POST /api/calculate/totals` 要求 Session、按当前有效部门规则确定的报销部门和 CSRF。请求不接受姓名、部门、总额、人民币大写或自动类型的覆盖天数/标准。
...
- Pinia 是当前编辑状态，SQLite 是已保存内容的权威来源。普通输入约 600 毫秒防抖自动保存；关联审批选择立即进入串行核验和保存，并优先于尚未提交的普通输入。页面显示保存状态；审批核验期间保持派生字段和布局稳定、暂时禁用重复选择，失败时显示真实错误。刷新或换设备按企业、员工和部门恢复最近报销，提交先保存最新输入；员工无需创建、保存或管理草稿。服务端 API 保留 `draftId` 命名用于兼容和并发控制。
- 提交成功可“再报销一笔”，明确 `FAILED_FINAL` 可“重新填写”，均保留已有记录；结果不确定时继续跟踪原任务。模板发布时间或公司、预算代码等选择项的集合、显示名变化时自动同步，不改变配置版本；当前草稿已选稳定值失效时只要求该草稿重新选择。映射控件的标题、业务别名或其他字段结构变化仍视为不兼容。尚未锁定且没有跟踪提交的内容遇到字段结构或映射变化，可在明确确认后“按新表单重新填写”，原记录保留，新表重新填写和上传材料。
...
- 付款要求由后端按员工确认的人民币票面 `amount > 500.00` 独立计算，不信任客户端必需标记，不使用原币数额、不除以 `receiptCount`，本期不引入分摊/部分报销金额或票据验真。铁路 `railType` 为 `high_speed/emu/regular/unknown`；仅 `category=rail_fare` 且 `high_speed` 暂时豁免。已识别高铁允许员工改成动车或普通列车并按非豁免处理；已识别动车或普通列车不能改成高铁取得豁免，未知类型和未分类/失败的 `other` 允许员工补选；已知非铁路证据不能伪标铁路取得豁免。复核和正式提交均严格校验。
- 新增出租车和国外票据本地解析器；国外票据保存 `originalCurrency/originalAmount`，并用 `originalDetailsEdited` 记录员工是否已经修正或清空原币字段，防止恢复或再次核验时被旧识别值覆盖。兼容该标记引入前保存的草稿：已保存原币值与当前识别证据不一致时按人工修正保留。人民币 `amount` 留给员工填写并确认 `cnyAmountConfirmed`。来源为 `foreign_receipt`、含外币警告或 `requiresCnyConfirmation` 时，即使币种未知也必须确认。
...
提交使用 `snapshotVersion=6`，冻结附件用途、证明材料关联、铁路类型、逐张出差审批对应的补助项、发起部门、来源类别和日期区间；上传记录严格为顺序 0 的 `GENERATED_PDF` 和顺序 1 的 `GENERATED_EXCEL`，OA 附件控件仅两项。汇总最多 500 页并受单对象容量限制；生成或上传失败使用持久检查点恢复，原件只有本地来源记录。

数据库迁移头为 `20260915_0019`。部署前应一致性备份 SQLite 与 staging，再通过原启动命令执行迁移并重启加载新代码。
