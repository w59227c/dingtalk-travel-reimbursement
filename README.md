...
- 查询并复核当前员工已通过的出差审批，将选中实例写入 OA 的 `RelateField`。多选审批必须公司、预算和来源出差类别一致，日期允许不连续；每张审批保留独立关联记录，重叠日期在补助计算和 Excel 明细中自动合并。审批详情读取在同一后端进程内跨用户统一限速；钉钉以 HTTP 429 或 `QpsLimitForAppkeyAndApi` 返回限流时，安全读取会有限退避重试，耗尽后明确提示接口繁忙，不再误报应用权限不足。
...
- 原始图片和 PDF 都由当前已鉴权页面读取为 Blob；预览弹窗立即打开，文件加载完成后，桌面端沿用浏览器内嵌 PDF 能力，移动端使用按需加载的 PDF.js 在当前页面 Canvas 中逐页绘制，不把受保护的 PDF URL 或 PDF iframe 交给钉钉 WebView。移动端缺少必要运行时能力或客户端加载、绘制失败时自动切换兼容预览，员工也可手动切换；兼容通道由服务器按需把当前页渲染为受限尺寸的 PNG，每次只生成当前页、复用有界文件校验进程槽，并对偶发的进程资源失败自动重试一次。
...
- `GET /api/reimbursements/drafts/{draftId}/files/{fileId}/content` 返回原件用于图片及 PDF 的鉴权 Blob 预览，校验企业、员工、部门、文件状态、有效期、大小和 SHA-256，响应禁止缓存；关闭或切换预览时释放 Blob URL。
...
- 后端结构化日志记录 request ID、方法、URL path、状态、耗时和安全错误类型；钉钉
  OpenAPI 最终失败时另记录固定操作类型、HTTP 状态和经过字符白名单过滤的上游错误码，
  以区分审批列表、审批详情、创建和附件操作。不记录钉钉响应正文、动态资源标识、查询
