...
- Pinia 是当前编辑状态，SQLite 是已保存内容的权威来源。普通输入约 600 毫秒防抖自动保存；关联审批选择立即进入串行核验和保存，并优先于尚未提交的普通输入。页面统一显示保存或核验状态。刷新或换设备按企业、员工和部门恢复最近报销，提交先保存最新输入；员工无需创建、保存或管理草稿。服务端 API 保留 `draftId` 命名用于兼容和并发控制。
