...
- 原始图片和 PDF 都由当前已鉴权页面读取并生成临时 Blob 地址；桌面和移动端的预览弹窗立即打开，文件加载完成后在弹窗内显示，不把受保护的 PDF URL 直接交给浏览器或钉钉 WebView。当前 PDF 查看器无法显示 Blob 时，员工可切换兼容预览，由服务器按需把当前页渲染为受限尺寸的 PNG；每次只生成当前页、复用有界文件校验进程槽，并对偶发的进程资源失败自动重试一次。
