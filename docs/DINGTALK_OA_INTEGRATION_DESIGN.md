...
员工选择审批后，前端从已经取得的同一审批候选同时展示所属公司、预算代码、出差类别和日期，并显示“正在核验”；这只是避免四项分批出现的即时反馈，不是权威保存结果。关联选择立即调用保存接口，不等待普通输入的 600 毫秒防抖，服务器仍按上述规则重新查询和复核。成功后切换为已核验的服务器值；失败时重新读取当前草稿并协调：若服务器实际已保存目标选择，则接受回读结果，否则恢复服务器原选择并显示真实错误。
