...
文档状态：与当前实现对齐，生产验收前基线；数据库迁移基线：`20260915_0019`；适用技术栈：Vue 3 + TypeScript、FastAPI、SQLite、钉钉企业内部应用。

## 1. 要实现的结果

员工从钉钉工作台打开“智能差旅报销”，在一个页面内完成以下事情：

1. 选择本人已通过的出差审批，由服务器确定当前报销部门并带入所属公司和预算代码，只读展示；预算代码的完整标签就是 Excel 内的项目，同一发起部门、公司、预算和出差类别的审批可多选；
...
| 模板指纹 | 对完整 Schema 计算的 SHA-256；发布时间、字段或选项变化时都会变化，用于审计，不单独作为结构不兼容的判据 |
...
`oa_template_profiles` 保存配置版本、完整 Schema、字段映射、确认指纹、允许的出差模板和确认人。草稿创建时保存当时的 `processCode + configVersion + schemaFingerprint`；其中配置版本表示管理员确认的结构与映射契约，完整指纹同时保留为审计证据。

页面读取选项、查询出差审批和正式提交前都会重新读取报销模板及所用出差模板。系统统一按结构契约分类变化：仅 `modifiedAt` 或选择项集合、显示名变化时自动保存最新完整 Schema 和指纹，不增加配置版本；映射字段的 ID、标题、业务别名、类型、可见/禁用状态、容器关系、日期格式，以及未映射必填字段或关联模板策略变化时才持久标记为 `DRIFTED` 并要求管理员重新确认。自动同步后仍会按最新选项逐草稿校验公司、预算代码和出差类别；当前稳定值被删除时只阻断受影响草稿并要求重新关联或重新选择。目录的自动同步和漂移标记使用完整旧状态作 CAS 条件，较早的远端检查不能覆盖较新的检查结果。已绑定旧结构版本的可编辑草稿不会静默改成新模板。

锁定快照保留报销模板的完整规范 Schema，并为每个出差模板保存紧凑结构指纹，不重复保存最多 20 份完整 Schema。快照序列化在锁定草稿前执行 4 MiB 聚合上限检查，超限时保持草稿可编辑并提示拆分材料。锁定后再次校验选择项时按稳定值判断；仅显示名或 option key 改名不会使已锁定任务失败，稳定值缺失、重复或映射结构变化仍会在上传前阻断。提交记录的 `schema_fingerprint` 保存实际锁定快照使用的最新完整指纹，草稿原始指纹继续作为其创建时绑定的审计信息。
...
服务器会用原查询窗口重新查询实例 ID，再读取详情，证明这张审批仍在当前员工可选列表中。服务器保存标题、业务编号、出差日期、审批发起部门、来源模板、Schema 指纹、查询窗口和复核时间，并解析当前公司与预算。多张审批必须同时满足审批发起部门、所属公司、预算代码和报销“出差类别”一致，日期允许不连续。不连续区间分别计算补助，直接或链式重叠的审批在计算与 Excel 明细阶段合并，避免重叠日期重复补助。

员工选择审批后，前端从已经取得的同一审批候选同时展示所属公司、预算代码、出差类别和日期，避免四项分批出现。服务器核验期间保持这些派生字段及页面布局稳定，暂时禁用重复选择，不额外插入“正在核验/已核验”瞬时标签，也不得误称已经进入提交阶段。关联选择立即调用保存接口，不等待普通输入的 600 毫秒防抖，服务器仍按上述规则重新查询和复核。成功后使用已核验的服务器值；失败时重新读取当前草稿并协调：若服务器实际已保存目标选择，则接受回读结果，否则恢复服务器原选择并显示真实错误。

审批发起部门是历史事实，报销归属部门始终来自员工当前有效部门，处理规则如下：

- 当前只有一个有效部门时直接使用该部门，不因审批由历史部门发起而阻断；
- 当前有多个有效部门且审批发起部门仍在其中时，自动使用该部门；
- 当前有多个有效部门且审批发起部门已不在其中时，员工只需为本次报销从当前部门中确认一次；候选排除名称去除首尾空白后以“其他”开头的部门，并按去除首尾空白后的名称去重；
- 当前部门列表中不存在的 ID 不能由前端指定，审批发起部门也不能被追加回当前部门；
- 已选审批只能继续关联相同发起部门的审批。前端提前禁用不同发起部门候选，后端保存和提交复核继续拒绝混合部门。

最终选定的当前部门用于草稿隔离、Excel 和 OA；审批发起部门只随关联审批保存并作为同部门校验依据。若员工当前部门发生变化，历史报销记录仍保留原归属，新报销按新的当前部门规则确定。
...
填写内容持久保存在 SQLite，按企业、员工和所选部门隔离。页面进入时自动准备空白报销或恢复最近记录；已提交的记录恢复进度或成功结果，避免刷新产生新审批。普通员工输入约 600 毫秒触发自动保存；关联审批选择立即核验和保存，并在串行保存中优先于尚未提交的普通输入。表单、文件用途和关联等业务修改都使用最新 `expectedRevision`。统一上传批次仅允许“上传下一份文件”和“首次识别已上传文件”两条受控通道重叠：上传仍按整份报销版本写入，首次 OCR 按文件状态及输入指纹保护，不推进整份报销版本；处理期间暂停表单修改和自动保存，避免相互覆盖。
...
统一上传、费用来源和行程单都复用持久 OCR 接口。统一上传在一次有界读取中完成分类和对应解析，复用提取文字；付款凭证可提取金额、日期和摘要供手工录入参考，不自动生成费用。人工明确上传的付款凭证和其他材料直接保存。服务器保存页面需要的结构化候选及 `NOT_REQUESTED/RUNNING/COMPLETE/FAILED` 状态。每条由 OCR 产生的费用行都通过 `sourceFileId` 保存来源文件 ID，不用金额、日期或说明文字反推关系。重新识别更新同一文件的候选，不重复增加明细；请求发出前或等待响应期间已经存在人工修改的费用行都保留现值，只更新文件证据并提示员工核对，不用新结果自动覆盖。
...
- OCR 识别的市内交通类型作为编辑默认值，不作为不可更改的事实；员工可改正为其他市内交通、出租车或网约车，前后端以员工最终保存的类型为准。`transportType=taxi/ride_hailing` 显示行程单关联入口。费用行通过 `itineraryFileIds` 保存活动的 `ATTACHMENT_ONLY + attachmentKind=itinerary` 文件，编辑窗口默认选择一个文件，需要时展开“补充行程单文件”；已有多文件关系继续完整显示和保存。
- 员工最终选择 `transportType=ride_hailing` 的费用必须关联至少一份行程单；前端提示缺失位置，后端复核和正式提交再次检查。持久字段 `requiresItinerary` 由最终交通类型统一派生：网约车为 `true`，出租车或其他类型为 `false`；旧数据只有该标记为 `true` 而缺少交通类型时规范为网约车，避免两个字段互相矛盾。OCR 未恢复具体说明时使用费用类别名称作为可提交的默认说明，并在列表和编辑窗口中一致显示。
- 后端按员工最终保存的 `transportType` 复核行程单要求：只把 `requiresItinerary` 改成 `false` 不能绕过仍为 `ride_hailing` 的费用，但员工明确把 OCR 默认类型改为 `taxi/other` 后不再沿用旧 OCR 标记。纸质出租车小票、火车票、住宿票据仍按对应类型处理。
...
国外票据的 `originalCurrency/originalAmount` 与人民币报销 `amount` 分开保存。OCR 不把外币数值直接写进人民币金额；员工填写人民币报销金额并确认 `cnyAmountConfirmed=true` 后才能提交。费用输入用严格布尔 `originalDetailsEdited` 记录员工是否已经修正或清空原币字段；未编辑时识别证据可补全原币信息，编辑后自动保存、刷新和服务端重新核验均保留人工结果。兼容该标记引入前保存的草稿：草稿已有原币值且与当前识别证据不一致时，前后端都将其推断为人工修改并补存该标记；完全没有原币值的旧草稿仍允许识别证据首次补全。该编辑意图不进入不可变费用快照。只出现 `$`、混合币种或日期顺序不明确时，保留相应警告和空值，不能猜成某个币种或日期。即使币种暂时未知，只要来源类型为 `foreign_receipt`、包含外币确认警告或 `requiresCnyConfirmation=true`，后端仍要求人民币金额确认。
...
费用 `railType` 枚举为 `high_speed/emu/regular/unknown`。目前仅 `category=rail_fare` 且类型明确为 `high_speed` 暂时豁免；动车、普通铁路及未知类型不自动豁免，最终铁路范围待业务确认。来源识别为高铁时，员工可核对原件后改成动车或普通列车，系统采用更严格的非豁免规则；来源识别为动车或普通列车时不能改成高铁取得豁免。未知/缺失及未分类或失败的 `categoryId=other` 允许员工补选；已知住宿、市内交通等非铁路来源不能通过伪标铁路类型获得豁免。
...
每张已关联出差审批保留一个原始补助输入，日期直接取自审批；不连续审批分别展示，直接或链式重叠的审批合并展示为一个补助时段。员工只确认每个最终时段的出发和返回是上午还是下午；出发/返回时段以中午 12:00 为半天边界。多张审批要求审批发起部门、所属公司、预算代码和来源出差类别完全相同。
...
- `snapshotVersion=6`、出差模板公司/预算来源映射、逐笔审批发起部门、来源出差类别、日期区间、逐审批补助项及整个规范 JSON 的 SHA-256。
...
| `POST /api/me/department/from-travel-approval` | 从已核验出差审批和当前有效部门解析本次报销部门；必要时返回当前部门候选 |
...
| 自动保存内容 | `input_json`、`related_instance_ids_json`；含 `editingState`、预算值、费用行、OCR 去向版本、来源/行程单/付款凭证文件 ID、`railType`、原币信息、原币字段人工编辑意图和人民币金额确认 |
...
- 日志只记录请求 ID、状态、内部业务 ID、错误码、耗时和异常类型；钉钉 OpenAPI 最终
  失败额外记录固定操作类型、HTTP 状态及经过字符白名单过滤的上游错误码，不记录响应
  正文、动态资源标识、票据全文、OCR 原文、密钥或签名材料；
...
- 启动前执行 Alembic 到 `20260915_0019`；readiness 会检查当前 revision 和关键字段。0016 保存逐笔已核对的来源出差类别，0017 清理已取消的 OA 冒烟测试配置，0018 新增管理员异常提交恢复审计，0019 保存关联审批的历史发起部门。升级先一致性备份 SQLite 和 staging，再通过原启动命令迁移/重启加载代码，保留全部业务记录与证据；0019 在已有发起部门证据时拒绝降级，0018 在存在恢复审计时拒绝降级，0015 在存在 `hotel_bill` 文件或 v4 提交时拒绝降级，0013 对非 `other` 用途或 v3 以上提交继续保留降级保护；
...
- 模板目录是否已经确认由管理员目录接口和员工 options 接口检查。员工 options、审批查询和正式提交在使用目录前重新读取报销与出差模板 Schema：发布时间和普通选项变化自动同步；结构契约不兼容才持久标记为 `DRIFTED`。目录兼容但当前草稿选项失效时，不锁定草稿、不创建提交任务，并提示员工重新关联或选择。
...
| 模板兼容状态为 `DRIFTED` | 字段结构、映射或关联策略不兼容，管理员重新 inspect、映射并确认；仅发布时间或普通选项变化不会进入此状态 |

现有应用日志包含 HTTP 请求 ID、状态和耗时，以及 worker、暂存回收、本地文件清理的错误事件。钉钉 OpenAPI 最终失败事件还包含 `errorCode`、`upstreamOperation`、`upstreamHttpStatus` 和 `upstreamErrorCode`，可区分审批列表、详情、创建或附件阶段；其中不包含请求参数、动态资源标识或响应正文。上线时应把这些结构化日志接入公司日志平台，并基于 SQLite 状态表补充仪表盘或定时巡检；当前代码没有独立的业务指标导出端点。
...
| 出差审批 | 当前员工隔离、已完成且同意、180 天业务窗口、钉钉单次 120 天分段、分页上限、来源字段精确解析/缺失/歧义、同发起部门/公司/预算/类别多选一致性、当前部门自动匹配与历史部门人工归属、候选过滤去重、客户端覆盖拒绝、清空保留材料、保存和提交再复核 |
...
- [ ] SQLite 已迁移到 `20260915_0019`，SQLite 与 staging 持久卷均可备份恢复；
