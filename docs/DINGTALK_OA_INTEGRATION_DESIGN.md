...
员工 API 不提供“重新创建”或“强制清理”按钮，避免在不确定状态下扩大损失。管理员可在系统设置的“异常提交恢复”中补录已核实的审批实例并恢复只读核对，或在明确确认 OA 未创建后进入孤儿附件清理；存在 commit 结果未知的附件时还必须二次确认远端文件不存在。两种处理均使用企业范围、CSRF、状态版本 CAS 和操作人记录约束。
